...
from lib.kodi_util import (xbmc, xbmcgui, ADDON, IPCTimeoutException, waitForGPEmpty, log,
...
def update_loop():
    ...
    if not FROM_KODI_REPOSITORY and getSetting('auto_update_check', True):
        ...
        check_interval = datetime.timedelta(hours=getSetting('update_interval_hours', 4))  # 4h, get
        check_immediate = getSetting('update_check_startup', True)  # get
        mode = getSetting('update_source', 'repository')  # get
